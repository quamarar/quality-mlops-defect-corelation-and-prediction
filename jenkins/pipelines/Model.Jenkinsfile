pipeline {
  ...
  environment {
    GIT_COMMIT_HASH = sh(script: 'git rev-parse --short HEAD')
          ...
          when {
            ...
            steps {
                dir ('model/preprocessing') {
                  ...
                  {
                     sh 'aws s3 sync . s3://msil-mvp-poc-apsouth1-internal/src/master/model/preprocessing/'
                     ...
                     sh 'aws ssm put-parameter --name "/mvp/development/training-job/ecr_preprocessing" --type "String" --value "731580992380.dkr.ecr.ap-south-1.amazonaws.com/msil-mvp-poc-apsouth1-preprocessing:${GIT_COMMIT_HASH}" --region "ap-south-1" --overwrite'
                }
            }
        }
        }
          ...
          when {
            ...
            steps {
                dir ('model/training') {
                  withAWS(roleAccount:'731580992380', role:'Cross-Account-role') 
                  {
                     sh 'aws s3 sync . s3://msil-mvp-poc-apsouth1-internal/src/master/model/training/'
                     ...
                     sh 'aws ssm put-parameter --name "/mvp/development/training-job/ecr_training" --type "String" --region "ap-south-1" --value "731580992380.dkr.ecr.ap-south-1.amazonaws.com/msil-mvp-poc-apsouth1-training:${GIT_COMMIT_HASH}" --overwrite'
                }
            }
        }
      }
