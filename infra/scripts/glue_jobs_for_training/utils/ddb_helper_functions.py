...
def get_algo_set_of_training_and_inferencing(mapping_json):
    ...
    """
    train_algos = reduce(operator.concat,
                         reduce(operator.concat, mapping_json['mapping_json_data']['TrainingMapping'].values()))
    inference_algos = reduce(operator.concat,
                             reduce(operator.concat, mapping_json['mapping_json_data']['InferenceMapping'].values()))
...
def delete_folder_from_s3(boto_s3_resource, bucket, marker):
    ...
    for folder in bucket.objects.filter(Marker=marker):
        # print(o.delete())
        folder.delete()
